...
var tableBody = document.getElementById('exchangeRatesBody');

data.forEach((item) => {
    var fields = item.fields;
    var row = tableBody.insertRow();
    row.insertCell().textContent = item.datasetid;
    row.insertCell().textContent = item.recordid;
    row.insertCell().textContent = fields.date;
    row.insertCell().textContent = fields.currency;
    row.insertCell().textContent = fields.rate.toFixed(4);
});